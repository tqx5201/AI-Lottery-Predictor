...
class PredictorWindow(QWidget):
    ...
    def save_history_data(self, lottery_type, period, data):
        ...
        try:
            ...
            print(f"历史数据已保存到数据库: {lottery_type}_{period}")
        except Exception as e:
            print(f"保存历史数据失败: {e}")
    ...
    def get_cached_history_data(self, lottery_type, period):
        ...
        try:
            ...
            if cached_data:
                print(f"使用数据库缓存的历史数据: {lottery_type}_{period}")
                self.update_cache_status("已缓存", "green")
                return cached_data
            else:
                print(f"数据库无缓存数据: {lottery_type}_{period}")
                self.update_cache_status("无缓存", "red")
                return None
                
        except Exception as e:
            print(f"获取缓存历史数据失败: {e}")
        ...
        try:
            ...
            try:
                self.db_adapter.save_prediction_result(
                    ...
                    extracted_numbers=None  # 二次预测暂不提取号码
                )
            except Exception as e:
                print(f"保存二次预测记录失败: {e}")
